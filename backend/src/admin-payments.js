...
module.exports = (pool) => {
  ...
  router.patch('/:id/status', async (req, res) => {
    try {
      ...
      if (payment.old_status !== status) {
        const title = status === 'verified' ? 'Payment verified' : 'Payment rejected';
        const message = status === 'verified' ? `Your payment of ₹${Number(payment.amount).toFixed(2)} for application ${payment.application_no} has been verified.` : `Your payment of ₹${Number(payment.amount).toFixed(2)} for application ${payment.application_no} has been rejected.${remarks ? ` Reason: ${remarks}` : ''}`;
