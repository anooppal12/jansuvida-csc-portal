...
let pool;
function getPool(){if(!pool)pool=mysql.createPool({host:process.env.DB_HOST,port:Number(process.env.DB_PORT||3306),user:process.env.DB_USER,password:process.env.DB_PASSWORD,database:process.env.DB_NAME,waitForConnections:true,connectionLimit:10,charset:'utf8mb4'});return pool;}
...
app.get('/api/health',async(_req,res)=>{try{await getPool().query('SELECT 1');res.json({ok:true,database:'connected'});}catch{res.status(503).json({ok:false,database:'unavailable'});}});
app.get('/api/services',async(_req,res)=>{try{const[rows]=await getPool().query(`SELECT s.id,s.name,s.slug,s.description,s.price,s.processing_time,c.name AS category FROM services s LEFT JOIN service_categories c ON c.id=s.category_id WHERE s.status=1 ORDER BY s.id DESC`);res.json({services:rows});}catch(error){console.error(error);res.status(500).json({error:'Unable to load services'});}});
